...
public class HotReferenceManager : Singleton<HotReferenceManager>
{
    ...
    public void RefreshAllQuestTriggerZones()
    {
        ...
        // 使用 FindObjectsOfType 包括未激活的物体（但注意：FindObjectsOfType 默认只返回激活的，要包括未激活需要 Resources.FindObjectsOfTypeAll 并过滤）
        CombatQuestTrigger[] Czones = Resources.FindObjectsOfTypeAll<CombatQuestTrigger>();
        List<CombatQuestTrigger> CvalidZones = new List<CombatQuestTrigger>();

        foreach (var zone in Czones)
        {
            // 只保留属于当前场景且未被销毁的实例
            if (zone != null && zone.gameObject.scene == SceneManager.GetActiveScene())
            {
                CvalidZones.Add(zone);
            }
        }

        Debug.Log($"HotReferenceManager: 找到 {CvalidZones.Count} 个 CombatQuestTrigger，开始刷新按钮引用...");

        foreach (var zone in CvalidZones)
        {
            zone.RefreshButtonReference();
        }
    }
